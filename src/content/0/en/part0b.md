...
Make sure that the <i>Network</i> tab is open, and check the <i>Disable cache</i> option as shown. checking <i>Hide extension URLS</i> is also useful (not shown in the picture above). <i>Preserve log</i> can also be useful: it saves the logs printed by the application when the page is reloaded.
